...
function Avatar({ rider, size }) {
  return (
    <div style={{
      ...
      {rider.photo
        ? <img src={rider.photo} alt={rider.name} loading="lazy" decoding="async" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        : rider.avatar}
    </div>
  )
}
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 4, marginBottom: 10 }}>
                          {bike.photos.slice(1).map(p => (
                            <div key={p.id} style={{ position: 'relative', aspectRatio: '1' }}>
                              <img src={p.url} alt="" onClick={() => setLightbox(p.url)} loading="lazy" decoding="async"
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
              {(rider.helmets || []).map(h => (
                <div key={h.id} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', position: 'relative' }}>
                  {h.photo ? (
                    <img src={h.photo} alt={h.name} onClick={() => setLightbox(h.photo)} loading="lazy" decoding="async"
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 4 }}>
              {rider.album.map(photo => (
                <div key={photo.id} style={{ position: 'relative', aspectRatio: '1' }}>
                  <img src={photo.url} alt="" onClick={() => setLightbox(photo.url)} loading="lazy" decoding="async"
                    style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: 6, cursor: 'pointer', display: 'block' }} />
                  {canEdit && (
                ...
                {trophies.map(c => (
                  <div key={c.id} onClick={onNavigateToChallenges}
                    ...
                    {c.awardType === 'banner' && c.bannerImage
                      ? <img src={c.bannerImage} alt="" loading="lazy" decoding="async" style={{ width: '100%', height: 90, objectFit: 'cover', display: 'block' }} />
    ...
    <div>
      {myPosts.map(post => (
        <div key={'p' + post.id} onClick={onNavigateToFeed} style={{ ...card, cursor: 'pointer' }}>
          ...
          {post.image && <img src={post.image} alt="" loading="lazy" decoding="async" style={{ width: '100%', maxHeight: 220, objectFit: 'cover', borderRadius: 8, display: 'block' }} />}
