...
export function buildNotificationFeed({ tasks = [], vehicles = [], services = [], docs = [], subs = [], people = [], expenses = [], valuables = [], appointments = [], reminders = [], t, lang = 'en', settings = {} }) {
  ...
  appointments.forEach(a => { const dd = daysUntil(a.date); if (dd != null && dd >= 0 && dd <= 7) { const who = (people.find(p => p.id === a.personId) || {}).name; out.push({ id: 'appt' + a.id, tint: dd <= 1 ? 't-warn' : 't-info', icon: 'calendar', title: a.title + (who ? ` · ${who}` : ''), meta: `${relativeDay(a.date, lang)}${a.time ? ' · ' + a.time : ''}`, go: 'appointments', sort: dd - 0.15, soon: dd <= 0 }) } })
  const open = tasks.filter(x => x.status !== 'completed' && x.status !== 'cancelled')
  open.filter(x => isOverdue(x.dueDate)).forEach(x => out.push({ id: 't' + x.id, tint: 't-danger', icon: 'clock', title: x.title, meta: t('overdue'), go: 'tasks/overdue', sort: -1, soon: true }))
  open.filter(x => isToday(x.dueDate)).forEach(x => out.push({ id: 'd' + x.id, tint: 't-info', icon: 'today', title: x.title, meta: t('todaysTasks'), go: 'tasks/today', sort: 0, soon: true }))
  ...
  open.filter(x => (x.assigneeId || x.assignedTo) && (isOverdue(x.dueDate) || isToday(x.dueDate) || x.status === 'waiting_someone'))
    .forEach(x => out.push({ id: 'a' + x.id, tint: isOverdue(x.dueDate) ? 't-danger' : 't-warn', icon: 'people', title: x.title, meta: `${t('waitingOn')} ${x.assignedTo || ''}`.trim(), go: 'tasks/delegated', sort: isOverdue(x.dueDate) ? -0.5 : 1.5, soon: isOverdue(x.dueDate) || isToday(x.dueDate) }))
  ...
  subs.filter(s => s.active !== false).forEach(s => { const dd = daysUntil(s.nextDue); if (dd != null && dd <= 5) out.push({ id: 'sub' + s.id, tint: dd <= 1 ? 't-danger' : 't-warn', icon: 'wallet', title: s.name, meta: `${t('nextDue')}: ${relativeDay(s.nextDue, lang)}`, go: 'subscriptions', sort: dd, soon: dd <= 0 }) })
  ...
  expenses.filter(e => e.method === 'installment' && Number(e.installmentMonths) > 0).forEach(e => {
    ...
    if (dd != null && dd >= 0 && dd <= 7) out.push({ id: 'inst' + e.id, tint: dd <= 2 ? 't-warn' : 't-info', icon: 'refresh', title: `${e.item || e.merchant || t('installment')} · ${paid + 1}/${months}`, meta: `${t('installment')}: ${relativeDay(next.toISOString(), lang)}`, go: 'moneycal', sort: dd + 0.05, soon: dd <= 0 })
  })
  people.forEach(pn => { const dd = daysToBirthday(pn.birthday); if (dd != null && dd <= 14) out.push({ id: 'bd' + pn.id, tint: 't-brand', icon: 'cake', title: `${pn.name} — ${t('birthdaySoon')}`, meta: dd === 0 ? relativeDay(new Date().toISOString(), lang) : relativeDay(new Date(Date.now() + dd * 86400000).toISOString(), lang), go: 'people', sort: dd + 0.1, soon: dd === 0 }) })
  valuables.forEach(v => { const dd = daysUntil(v.warrantyExpiry); if (dd != null && dd >= 0 && dd <= 30) out.push({ id: 'war' + v.id, tint: dd <= 7 ? 't-warn' : 't-info', icon: 'gift', title: `${v.name} — ${t('warranty')}`, meta: `${t('warranty')}: ${relativeDay(v.warrantyExpiry, lang)}`, go: 'valuables', sort: dd + 0.2, soon: dd <= 0 }) })
  // Identity-document renewals (Iqama, passport, license, national ID) within 60 days.
  people.forEach(pn => PERSON_DOC_FIELDS.forEach(df => { const dd = daysUntil(pn[df.field]); if (dd != null && dd <= 60) out.push({ id: df.field + pn.id, tint: dd < 0 ? 't-danger' : dd <= 21 ? 't-warn' : 't-info', icon: 'shield', title: `${pn.name} — ${t(df.key)}`, meta: dd < 0 ? t('expired') : `${t('renews')}: ${relativeDay(pn[df.field], lang)}`, go: 'renewals', sort: dd - 0.3, soon: dd <= 0 }) }))
