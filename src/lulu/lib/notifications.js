...
export function buildNotificationFeed({ tasks = [], vehicles = [], services = [], docs = [], subs = [], people = [], expenses = [], valuables = [], appointments = [], reminders = [], t, lang = 'en', settings = {} }) {
  ...
  open.filter(x => x.status === 'waiting_me').forEach(x => out.push({ id: 'w' + x.id, tint: 't-warn', icon: 'flag', title: x.title, meta: t('waitingForMe'), go: 'tasks/waiting_me', sort: 1, now: true }))
  ...
  expenses.filter(e => e.method === 'installment' && Number(e.installmentMonths) > 0).forEach(e => {
    ...
    if (dd != null && dd >= 0 && dd <= 7) out.push({ id: 'inst' + e.id, tint: dd <= 2 ? 't-warn' : 't-info', icon: 'refresh', title: `${e.item || e.merchant || t('installment')} · ${paid + 1}/${months}`, meta: `${t('installment')}: ${relativeDay(next.toISOString(), lang)}`, go: 'moneycal', sort: dd + 0.05, soon: dd <= 0, now: dd <= 0 })
  })
  people.forEach(pn => { const dd = daysToBirthday(pn.birthday); if (dd != null && dd <= 14) out.push({ id: 'bd' + pn.id, tint: 't-brand', icon: 'cake', title: `${pn.name} — ${t('birthdaySoon')}`, meta: dd === 0 ? relativeDay(new Date().toISOString(), lang) : relativeDay(new Date(Date.now() + dd * 86400000).toISOString(), lang), go: 'people', sort: dd + 0.1, soon: dd === 0, now: dd === 0 }) })
