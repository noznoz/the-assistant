...
export function maybeSeed() {
  ...
  const mFaisal = db.insert('members', { name: 'Faisal Al-Harbi', role: 'head', departmentId: opsDept.id, mobile: '+966500000031', whatsapp: '+966500000031' })
  const mSara = db.insert('members', { name: 'Sara Khan', role: 'manager', departmentId: opsDept.id, mobile: '+966500000032', whatsapp: '+966500000032', reportsToId: mFaisal.id })
  db.insert('members', { name: 'Yousef Ali', role: 'officer', departmentId: opsDept.id, mobile: '+966500000034', reportsToId: mSara.id })
  db.update('departments', opsDept.id, { headId: mFaisal.id })
  const salesDept = db.insert('departments', { name: 'Sales', note: 'Commercial team' })
  const mOmar = db.insert('members', { name: 'Omar Nasser', role: 'head', departmentId: salesDept.id, mobile: '+966500000033', whatsapp: '+966500000033' })
