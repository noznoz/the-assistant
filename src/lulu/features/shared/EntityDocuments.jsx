...
// Documents linked to any entity via a foreign-key field (e.g. vehicleId,
// propertyId). Reuses the central document vault so the files also show there.
export default function EntityDocuments({ filterKey, id, hint }) {
  const { t, lang } = useT()
  ...
  const mine = docs.items.filter(d => d[filterKey] === id)
  ...
  return (
    <>
      <Button block variant="primary" icon="plus" onClick={() => setEditor({ [filterKey]: id })}>{t('addDocument')}</Button>
      {mine.length === 0 ? (
        <Empty icon="doc" title={t('noVehicleDocs')} text={hint || t('vehicleDocsHint')} />
      ) : (
      ...
      {editor && (
        <DocEditor initial={editor.id ? editor : { [filterKey]: id }} onClose={() => setEditor(null)} onSaved={() => toast.show(t('savedToast'))} onToast={toast.show} />
      )}
      {toast.node}
    </>
  )
